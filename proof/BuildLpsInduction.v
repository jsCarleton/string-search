...
(** [_bare] version, mirroring [BuildLpsIterate.v]'s pattern: the same
    fact without the [AI_frame 0 _ f0] wrapping, built by calling
    [build_lps_iterate_match_bare]/[_backtrack_bare]/[_giveup_bare]
    instead of the framed originals (everything else -- the fuel
    induction, the [cand_fuel]-matching case split, the memory-length/
  ...
  - simpl.
    ...
    + (* Match: [cl = ci], result is [S len]. *)
      ...
      have [s' [m' [Hlkm' [Hwr' Hred]]]] :=
        build_lps_iterate_match_bare hs s inst patPtr patLenN lpsPtr (Z.of_nat len) (Z.of_nat i) tmpN memaddr m ci
    ...
    + (* Mismatch: [cl <> ci]. *)
      ...
      * (* len = 0: give up, result 0. *)
        ...
        have [s' [m' [Hlkm' [Hwr' Hred]]]] :=
          build_lps_iterate_giveup_bare hs s inst patPtr patLenN lpsPtr (Z.of_nat i) tmpN memaddr m ci cl
      ...
      * (* len = S len': backtrack to [table[len']]. *)
        ...
        have Hback := build_lps_iterate_backtrack_bare hs s inst patPtr patLenN lpsPtr (Z.of_nat (S len'))
          (Z.of_nat i) tmpN memaddr m ci cl (Z.of_nat v)
