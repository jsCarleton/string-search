...
Proof.
  move=> hs s inst patPtr patLenN lpsPtr memaddr m p table i f0
    Hpp Hlp HpatLen Hlenp Hlenp4 Hpatb Hlpsb Hmems Hlkm Hmempat Hmemlps Hpmm Hilt Htlen Htcb.
  ...
  - simpl.
    ...
    + (* Mismatch: [cl <> ci]. *)
      ...
      * (* len = S len': backtrack to [table[len']]. *)
        ...
        move: HrIH => [s'' [m'' [tmpN'' [Hlkm'' [Hlmm'' Hred'']]]]].
        exists s'', m'', tmpN''.
        rewrite Hvdef.
        split; [exact Hlkm'' |]. split; [exact Hlmm'' |].
