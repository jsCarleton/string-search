...
(** [_bare] version, mirroring [BuildLpsIterate.v]/[BuildLpsInduction.v]'s
    pattern: the same induction, without the [AI_frame 0 _ f0] wrapping,
    built from [build_lps_group_fuel_bare] and [build_lps_exit_bare]
...
Proof.
  ...
  - (* k = 0: i = length p, the loop exits. *)
    ...
    have Hred := build_lps_exit_bare hs s inst patPtr patLenN lpsPtr (Z.of_nat len) (Z.of_nat i) tmpN
